...
from prompt_types import Prompt, Completion
from query_clients import LLMClient
from python_utils import chunk, remove_from_str
...
class CompletionCache:
    def __init__(self, cache_file: Optional[str]) -> None:
        ...
        if not self.disabled:
            ...
            self.temp_cache_file = os.path.join(
                os.path.dirname(self.cache_file), "temp_" + os.path.basename(self.cache_file)
            )
    ...
    def query(
        ...
        if cached_completion is not None:
            cached_completion = Completion(
                cached_completion["text"], cached_completion["cum_logprob"], cached_completion["num_tokens"]
            )
    ...
    def update(self, query_idx_str: str, completion: Completion) -> bool:
        ...
        self.current_cache[query_idx_str] = {
            "text": completion.code,
            "cum_logprob": completion.cumulative_logprob,
...
class LLMQuerier(ABC):
    def __init__(
        self, log_directory: Optional[str], cache_file: Optional[str], global_batch_size: Optional[int]
    ) -> None:
    ...
    def generate_with_info(
        ...
        with tqdm(total=len(prompts)) as pbar:
            ...
            for i, prompt_chunk in enumerate(chunked_prompts):
                completions, cost = _generate_completions(
                ...
                if self.clients[client_name].model_is_o1:
                    ...
                    if len(failed_prompts):
                        if o1_retry:
                            print(
                                f"{len(failed_prompts)} invalid prompt errors... Requerying with filter {O1_FILTER}! {curr_hash}"
                            )
                            filtered_prompts = []
                            for prompt in failed_prompts:
                                ...
                                for msg in prompt:
                                    new_prompt.append(
                                        {"role": msg["role"], "content": remove_from_str(msg["content"], O1_FILTER)}
                                    )
                                filtered_prompts.append(new_prompt)
                            new_completions = self.generate_with_info(
                            ...
                            print(
                                f"{len(failed_prompts)} invalid prompt errors... Requerying with {DEFAULT_O1_REPLACEMENT}! {curr_hash}"
                            )
                            new_completions = self.generate_with_info(
                                client_name=DEFAULT_O1_REPLACEMENT,
        ...
        if total_failed_prompts:
            ...
            print(
                f"an example completion: {all_completions[0].code[:200]}\n...\n{all_completions[0].code[-200:]} {curr_hash}"
            )
    ...
    def log(
        ...
        if self.log_directory is not None:
            ...
            output_file = os.path.join(
                self.log_directory, log_name + datetime.datetime.now().strftime("%m-%dT%H:%M:%S") + ".json"
            )
...
def _generate_completions(
    client: LLMClient,
    messages: Union[list[Prompt], tuple[Prompt, ...]],
    ...
    for i, prompt in enumerate(messages):
        ...
        completion, cache_key = cache.query(
    ...
    generated_completions, cost = client.generate_completions(
        messages=to_generate,
        frequency_penalty=frequency_penalty,
        logit_bias=logit_bias,
        max_tokens=max_tokens,
        presence_penalty=presence_penalty,
...
if __name__ == "__main__":
    ...
    llmq = LLMQuerier("temp_logs", None, 1024)
    ...
    # print(
    #     llmq.generate(
    #         "model_configs/gpt-4o.json",
    #         [
    #             [
    #                 {"role": "system", "content": "You are an expert counter."},
    #                 {"role": "user", "content": "Please count to 10."},
    #             ],
