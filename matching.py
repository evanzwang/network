...
class Matcher:
    ...
    def elo_query(self, query_str: str, k: int, num_pairs: int = 100) -> list[Person]:
        ...
        queries = [prompts.get_comparison_prompt(query_str, self.people[p1], self.people[p2]) for p1, p2 in pairs]
        outputs = self.generate(queries)
        winning_names = [prompts.parse_simple_prompt_response(output)[1] for output in outputs]

        ratings = {name: 1500.0 for name in self.people}
        elo.update_elo_batch(ratings, [(p1, p2, win.lower()) for (p1, p2), win in zip(pairs, winning_names)])
