...
class LLMClient:
    ...
    def num_tokens_for_convo(
        self, convo: Prompt, encoding: tiktoken.Encoding = tiktoken.get_encoding("cl100k_base"), approx: bool = False
    ) -> int:
        num_tokens = 0
        content_str = []
        if self.is_chat:
...
class OpenAIClient(LLMClient):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            price_per_input_output=price_per_input_output,
        )
...
#     def _generate_vLLM_chat_completions(
#         self,
...
#             completions.append(
#                 Completion(
#                     output.outputs[0].text, output.outputs[0].cumulative_logprob, len(output.outputs[0].token_ids)
#                 )
#             )
...
#                 print(
#                     f"SGLangClient: OpenAI API connection error. Make sure SGLang server is running at {self.base_url}"
#                 )
...
#     def _generate_llm_engine_chat_completion(
#         self,
...
#     def _generate_llm_engine_completion(
#         self,
#         prompt: str,
#         frequency_penalty: Optional[float],
#         max_tokens: Optional[int],
...
#             try:
#                 start = time.time()
#                 completion = llmengine.Completion.create(
#                     model=self.model_name,
#                     prompt=prompt,
#                     frequency_penalty=frequency_penalty,
...
#         num_workers: Optional[int] = None,
#         price_per_input_output: tuple[float, float] = (0.0, 0.0),
#         start_backoff: float = 21.0,
#         max_backoff: float = 1.5 * 60,
#     ) -> None:
#         super().__init__(
#             model_name=model_name,
#             is_chat=is_chat,
...
if __name__ == "__main__":
    ...
    print(
        lc.generate_completions(
            [[{"role": "user", "content": "what is up my dude?"}]] * 100,
